...
# ---------------------------------------------------------------------------
# data/hard_negatives.jsonl — generated by build_hard_negatives.py
# ---------------------------------------------------------------------------

def _hard_negatives():
    import json
    if not os.path.exists(td.HARD_NEGATIVES):
        pytest.skip("hard negatives not generated")
    with open(td.HARD_NEGATIVES, encoding="utf-8") as fh:
        return [json.loads(line) for line in fh if line.strip()]


def test_hard_negatives_are_all_benign_and_grouped():
    rows = _hard_negatives()
    assert rows
    assert {r["label"] for r in rows} == {0}
    assert all(r.get("group") for r in rows), "CV needs a group per row"


def test_hard_negatives_exclude_every_held_out_prompt():
    reserved = td.held_out_texts()
    leaked = [r["text"] for r in _hard_negatives()
              if td._normalize(r["text"]) in reserved]
